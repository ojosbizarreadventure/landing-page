...
<script lang="ts">
	...
	const plans: Record<string, { name: string; m: number; a: number; mLabel: string; aLabel: string; suffix: string; minSeats: number; features: string[] }> = {
		plus: {
			name: 'Plus', m: 1300, a: 850, mLabel: '₹1,300', aLabel: '₹850', suffix: '/user/mo', minSeats: 3,
		...
		ultra: {
			name: 'Ultra', m: 2500, a: 1625, mLabel: '₹2,500', aLabel: '₹1,625', suffix: '/user/mo', minSeats: 1,
		...
		starter: {
			name: 'Starter', m: 2999, a: 2399, mLabel: '₹2,999', aLabel: '₹2,399', suffix: '/mo', minSeats: 1,
			features: ['Up to 10 users', '5 active projects', 'Full CRM + Projects', 'OJO AI \u00b7 500 credits/mo', 'Client portal', 'Email support']
		},
		growth: {
			name: 'Growth', m: 9999, a: 7999, mLabel: '₹9,999', aLabel: '₹7,999', suffix: '/mo', minSeats: 1,
		...
		<div class="container" style="max-width:640px">
			<div class="checkout-form" style="margin-bottom:24px">
				...
				<div style="display:flex;flex-direction:column;gap:10px;margin-bottom:24px">
					...
					{#if plan.minSeats > 1}
						<div style="display:flex;justify-content:space-between;padding:12px 16px;background:var(--sf);border:1px solid var(--bd);border-radius:10px">
							<span style="font-size:14px;color:var(--t2)">Minimum seats</span>
							<span style="font-size:14px;font-weight:700">{plan.minSeats}</span>
						</div>
					{/if}
